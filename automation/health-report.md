...
- **Intégrité des liens internes (disque)** : 94 fichiers `.html` passés en revue (hors `/sw/`, dont 93 pages de contenu + 1 fichier de vérification Google `google5f267a48e7657a47.html`), tous les `href`, `src` d'image, `background-image: url(...)`, liens JSON-LD (`"url"`) et balises `hreflang` internes vérifiés — 0 lien cassé, 0 image manquante.
