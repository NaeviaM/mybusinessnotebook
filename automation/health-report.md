...
- **Vérification live impossible depuis cet environnement.** Le proxy sortant du bac à sable a refusé la connexion HTTPS vers `mybusinessnotebook.com` (`CONNECT` → `403`, confirmé à la fois par WebFetch et par `curl`, et par le statut du proxy : `connect_rejected` / "gateway answered 403 to CONNECT"). Ce n'est pas un défaut du site, c'est une restriction de cet environnement d'exécution — les sections A (disponibilité live) et B (balayage live du sitemap, ~78 URLs) n'ont pas pu être exécutées aujourd'hui.
- **DNS : aucun problème.** Résolution via `python3 socket.getaddrinfo` (ne passe pas par le proxy HTTPS bloqué) :
  ...
  - `www.mybusinessnotebook.com` → `76.76.21.123` / `66.33.60.35` (plages Vercel)
